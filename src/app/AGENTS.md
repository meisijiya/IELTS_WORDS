...
- `/login`、`/register`、`/admin/invites`、`/leaderboard` 是多用户引入的新增页面;`/admin/*` 页面在 RSC 层 `await requireAdmin()` 后才渲染,未登录或非 admin 跳回 `/login`。
- `/register?code=xxx` 接收邀请码,RSC 校验后渲染表单(防直接 GET 跳过邀请)。
- `/leaderboard` 是 RSC 取全员 stats 后交客户端,客户端按 range 切换 + 卡片点击展开。
- `/settings` 现在含四个 sections:发音 / 拉取优先级 / 练习音效 / **账号**(改 username + 改密码)。账号 section 调 `PUT /api/users/me`,接受 `{username?, password, newPassword?}`。

## 排行榜重复实现 (技术债)

`/leaderboard` 页面直接调 `src/lib/leaderboard.ts: getLeaderboard()`;但 `/api/leaderboard` 路由手写了一套几乎相同的查询逻辑,**没有**复用 lib。新增 leaderboard 相关 route 必须直接 `getLeaderboard(me.id)`,不要复制粘贴。修复优先级 P2。
