...
- `/login`、`/register`、`/admin/users`、`/admin/invites`、`/leaderboard` 是多用户引入的新增页面;`/admin/*` 页面在 RSC 层 `await requireAdmin()` 后才渲染,未登录或非 admin 跳回 `/login`。
- `/register?code=xxx` 接收邀请码,RSC 校验后渲染表单(防直接 GET 跳过邀请)。
- `/leaderboard` 是 RSC 取全员 stats 后交客户端,客户端按 range 切换 + 卡片点击展开。
- `/settings` 现在含三个 sections:发音 / 拉取优先级 / **账号**(改 username + 当前密码)。账号 section 调 `PUT /api/users/me`。
